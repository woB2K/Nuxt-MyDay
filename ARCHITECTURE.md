...
- `GET /api/finance/savings` — единственное место, где сервер ещё считает «этот месяц» от собственного `new Date()` (нарушает решение о таймзонах), и не валидирует `page`/`limit`: `?page=abc` даёт `skip: NaN` → 500. Чинится в **2.23** вместе с периодом в истории накоплений
- `ResultSummary` в All Transactions (дизайн v3) требует income/expense по **отфильтрованному** набору, а `GET /api/finance/transactions` возвращает только `total`. Решить в **2.20/2.21**: рекомендация — научить `GET /api/finance/summary` принимать те же `type`/`categoryIds`/`search` (hero остаётся без фильтров, strip запрашивается с ними)
...
- `PATCH`/`DELETE` одиночных ресурсов отдают 500 вместо 404, когда записи нет (Prisma P2025 наружу) — заодно с **2.18**
