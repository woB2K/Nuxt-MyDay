...
### Обратная связь: тост сверху, ошибка под полем

**Тост живёт сверху экрана, и это не вкусовщина.** По `DESIGN.md` он снизу, но на телефоне форму заполняют с открытой клавиатурой — она занимает нижнюю половину экрана и накрывает тост целиком. Пользователь жмёт «Сохранить», тост показывается и гаснет за три секунды за клавиатурой; снаружи это выглядит как «тостов в приложении нет вообще». В эмуляции баг не воспроизводится — там клавиатуры не существует. Отсюда `top-safe` вместо `bottom-*` и анимация сверху вниз.
