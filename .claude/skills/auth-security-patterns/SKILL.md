...
**401 зарезервирован за сессией.** Раз `useApi()` трактует 401 как «обнови токен и повтори», бизнес-логика внутри защищённых эндпоинтов им пользоваться не может: неверный PIN или пароль — это **403** (сессия валидна, секрет не подошёл). Иначе опечатка пользователя вызывает ротацию refresh-токена и дублирует запрос.

**Неудачные попытки секрета считает `assertPin()`, не middleware.** Путь не знает исхода: на уровне middleware либо успешные операции тратят лимит, либо часть каналов (`DELETE /api/settings/pin`) остаётся открытой для перебора. Бакет `<userId>:secret` общий для verify / disable / change / reset и тратится только на неудачах.
